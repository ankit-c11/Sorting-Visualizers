...
class InsertionSort extends React.Component{
    state={numbers:[5,2,3,6,19,15,11,7,4,24,8,12,1,4,18],i:1,j:0,n:15}
    
    startSort = () => {
      if(this.intervalId){
        return;
      }
      var {numbers,i} = this.state;
      var ele = numbers[i];
      this.setState({ele})
      this.intervalId = setInterval(this.insertionSort,500)
    }
    
    insertionSort = () => {
      var {numbers,i,j,n,ele} = this.state
      if(i === n){
          this.setState({j:-1})
          clearInterval(this.intervalId)
      }
      else{
       if(j >= 0 && numbers[j] > ele){
          numbers[j+1] = numbers[j];
          j--;
          this.setState({numbers,i,j,n})
        }
        else{
          numbers[j+1] = ele;
          i++;
          j=i-1;
          ele = numbers[i];
          this.setState({numbers,i,j,n,ele})
        } 
      }
    }
    ...
    render() {
      var jsx = this.state.numbers.map((n, i) => {
        var {j} = this.state;
        var backgroundColor = j === i ? "#c23030" : "#4392ba";
        var border= "solid 2px #f1f1f1";
      ...
      return (
        <div className='main-div' style={style}>
          ...
          <Button startSort={this.startSort} title={'Insertion Sort'}/>
        </div>
     );
    }
}
